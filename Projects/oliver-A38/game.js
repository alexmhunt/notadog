...
const W = ( function () {
	...
	let _actor_delta = 255; // current actor delta
	...
	const _LEVELS = [
		...
		// Level 20
		{
			...
			fade_walls : [
				[ 3, 2, _STEP1 ], [ 5, 2, _STEP2 ], [ 5, 5, _STEP3 ], [ 3, 5, _STEP4 ],
				[ 3, 3, _STEP5 ], [ 4, 2, _STEP6 ], [ 5, 4, _STEP7 ], [ 4, 5, _STEP8 ],
				[ 3, 4, _STEP9 ], [ 5, 3, _STEP10 ], [ 8, 8, _STEP11 ], [ 9, 8, _STEP12 ],
				[ 8, 9, _STEP13]
			],

			fade_foods : [
				[ 5, 7, _STEP1 ], [ 6, 7, _STEP2 ], [ 7, 7, _STEP3 ], [ 8, 5, _STEP4 ],
				[ 7, 3, _STEP5 ], [ 7, 1, _STEP6 ], [ 5, 1, _STEP7 ], [ 2, 1, _STEP8 ],
				[ 1, 3, _STEP9 ], [ 1, 6, _STEP10 ], [ 1, 8, _STEP11 ], [ 4, 7, _STEP12 ]
			],
	...
	const move = function ( x, y ) {
		...
		if ( data === _IS_FOOD ) {
			...
			if (_level === 20){
				_actor_delta += 160; //make actor more opaque
			}
			...
			if (_level === 20){
				_actor_delta -= 20; // lose visibilty of actor
				_actor_size += 1; // shrink actor
			} else {
				_actor_size += 2; // shrink actor
			}
			if (_level == 20 && _actor_delta < -40){
				PS.audioPause( _channel );
				PS.audioPlay("perc_triangle", {volume: .1}) 
				_play = false;
