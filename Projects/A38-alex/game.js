...
const W = ( function () {
	...
	const _startEnding = function(){
		// fade the player away
		const fadeActor = function(){
			_place_actor( _actor_x, _actor_y, _actor_size );
			_actor_size += 2;
			//PS.debug("fading actor")
			if(_actor_size > 20){
				PS.timerStop(actorFadeTimer);
				fadeTimer = PS.timerStart(60, fadeAway);
				_place_floor( _actor_x, _actor_y );
				PS.border(_actor_x, _actor_y, 0)
			}
		}
		...
		let color = []
		PS.unmakeRGB(PS.COLOR_GRAY, color)

		let actorFadeTimer = PS.timerStart(60, fadeActor);
		let fadeTimer;
